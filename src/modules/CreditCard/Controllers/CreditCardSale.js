const { CreditCardSale, Op } = require('../../../database');

/** Function for getting all credit transfers in the system */
exports.getAll = function (req, res) {
    CreditCardSale.findAll()
        .then(cards => {
...
exports.create = function (req, res) {
    if (req.body.number && req.body.amount) {
        CreditCardSale.create(req.body)
            .then(card => {
                CreditCardSale.findById(card.id)
                    .then(foundCard => {
...
exports.get = function (req, res) {
    CreditCardSale.findById(req.params.id)
        .then(card => {
            if (!card) {
                return res.status(404).json({
                    error: {
...
exports.delete = function (req, res) {
    CreditCardSale.destroy({ where: { id: req.params.id } })
        .then(card => {
            if (!card) {
                return res.status(404).json({
                    error: {
